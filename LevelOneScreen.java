...
public class LevelOneScreen implements Screen {
    ...
    LevelOneScreen(MySpaceFarceGame game, int score, int lives) {
        this.game = game;
        this.score = score;
        this.lives = lives;
        ...
        backgrounds[0] = textureAtlas.findRegion("black1");
        ...
        enemySmallShipTextureRegion =  textureAtlas.findRegion("ufoYellow");
        enemyMediumShipTextureRegion = textureAtlas.findRegion("enemyBlue1");
        enemyBossShipTextureRegion = textureAtlas.findRegion("enemyRed2");
        enemyShieldTextureRegion = textureAtlas.findRegion("shield1");
        enemyShieldTextureRegion.flip(false, true);

        // enemy lasers
        smallEnemyLaserTextureRegion = textureAtlas.findRegion("laserRed10");
        mediumEnemyLaserTextureRegion = textureAtlas.findRegion("laserBlue03");
        bossEnemyLaserTextureRegion = textureAtlas.findRegion("laserRed14");

        // extra regions
        explosionTexture = new Texture("Explosion.png");
        mediumAsteroidTextureRegion = textureAtlas.findRegion("meteorBrown_med3");
        bigAsteroidTextureRegion = textureAtlas.findRegion("meteorBrown_big1");
        ...
        playerShip = new PlayerShip(WORLD_WIDTH / 2, WORLD_HEIGHT / 4,
                10, 10, 48, 3, 1f, 4,
                45, 0.5f, 1,
                playerShip1TextureRegion, playerShieldTextureRegion, playerLaserTextureRegion);
    ...
    public void render(float deltaTime) {
        ...
        // spawn enemy ships
        spawnEnemyShips(deltaTime, 10, 1.4f,
                4,5.4f,
                1, 11.4f);
        ...
        // spawn astroids
        spawnAsteroids(deltaTime, 10, 2.4f,
                5, 3.7f);
        ...
        // spawn power ups
        spawnPowerUps(deltaTime, 2, 7.5f, 3,
                11.5f, 7, 8.5f);
        ...
        if(enemiesKilled == 15) {
            ...
            if (Gdx.input.isKeyJustPressed(Input.Keys.ENTER)) {
                game.setScreen(new LevelTwoScreen(game, score, lives, lifeFactor, playerShip2TextureRegion));
                dispose();
            }
        }
    }
    private void updateAndRenderHUD() {
        ...
        // add a life when score reaches 500 times the life factor times 1.6
        ...
        // if the player has zero shield and life ends the game
    ...
    private void spawnGreenPill(float deltaTime, int greenPillFinalCount, float timeBetweenGreenPillSpawns) {// adds to life number
        ...
        if(greenPillSpawnTimer > timeBetweenGreenPillSpawns) {
            ...
            if(greenPillCount  < greenPillFinalCount) {
                powerUpList.add(new PowerUp(MySpaceFarceGame.random.nextFloat() * (WORLD_WIDTH - 10) + 3,
                        WORLD_HEIGHT - 3, 3, 4, 40, "pill", greenPillTextureRegion));
        ...
        if(enemySpawnTimer > timeBetweenBossSpawns) {
            ...
            if(bossShipCount < bossShipFinalCount) {
                enemyShipList.add(new EnemyShip(MySpaceFarceGame.random.nextFloat() * (WORLD_WIDTH - 20) + 5,
                        WORLD_HEIGHT - 5, 20, 20, 30, 10, 0.8f, 3,
                        20, 1.1f, 700, "boss", 1,
        ...
        while(powerUpListIterator.hasNext()) {
            ...
            if(playerShip.intersects(powerUp.boundingBox)) {
                ...
                if(powerUp.getName() == "pill" && powerUp.intersects(playerShip.boundingBox)) {
                    ...
                    playerShip.setLasers(fire5TextureRegion, playerShip.getPower() + 1);
                    playerShip.update(deltaTime);
                    powerUpList.remove();
                    break;
                }
