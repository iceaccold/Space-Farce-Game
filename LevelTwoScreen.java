...
public class LevelTwoScreen implements Screen {
    ...
    LevelTwoScreen(MySpaceFarceGame game, int score, int lives, int lifeFactor, TextureRegion playerShipTextureRegion) {
        this.game = game;
        this.score = score;
        this.lives = lives;
        this.lifeFactor = lifeFactor;
        this.playerShipTextureRegion = playerShipTextureRegion;
        ...
        backgrounds[0] = textureAtlas.findRegion("blue1");
        ...
        enemySmallShipTextureRegion =  textureAtlas.findRegion("enemyBlue1");
        enemyMediumShipTextureRegion = textureAtlas.findRegion("enemyRed2");
        enemyBossShipTextureRegion = textureAtlas.findRegion("ufoYellow");
        enemyShieldTextureRegion = textureAtlas.findRegion("shield1");
        enemyShieldTextureRegion.flip(false, true);

        // enemy lasers
        smallEnemyLaserTextureRegion = textureAtlas.findRegion("laserBlue03");
        mediumEnemyLaserTextureRegion = textureAtlas.findRegion("laserRed14");
        bossEnemyLaserTextureRegion = textureAtlas.findRegion("laserRed10");
    ...
    public void render(float deltaTime) {
        ...
        // spawn enemy ships
        spawnEnemyShips(deltaTime, 12, 2.9f,
                6,6.9f,
                3, 11.9f);
        ...
        if(enemiesKilled == 21) {
            ...
            if (Gdx.input.isKeyJustPressed(Input.Keys.ENTER)) {
                game.setScreen(new LevelThreeScreen(game, score, lives, lifeFactor, playerShip3TextureRegion));
                dispose();
            }
        }
    }
    ...
    private void spawnGreenPill(float deltaTime, int greenPillFinalCount, float timeBetweenGreenPillSpawns) {
        ...
        if(greenPillSpawnTimer > timeBetweenGreenPillSpawns) {
            ...
            if(greenPillCount  < greenPillFinalCount) {
                powerUpList.add(new PowerUp(MySpaceFarceGame.random.nextFloat() * (WORLD_WIDTH - 10) + 3,
                        WORLD_HEIGHT - 3, 3, 4, 44, "pill", greenPillTextureRegion));
        ...
        if(enemySpawnTimer > timeBetweenBossSpawns) {
            ...
            if(bossShipCount < bossShipFinalCount) {
                enemyShipList.add(new EnemyShip(MySpaceFarceGame.random.nextFloat() * (WORLD_WIDTH - 20) + 5,
                        WORLD_HEIGHT - 5, 20, 20, 32, 12, 1.5f, 3,
                        20, 1f, 770, "boss", 2,
        ...
        while(powerUpListIterator.hasNext()) {
            ...
            if(playerShip.intersects(powerUp.boundingBox)) {
                ...
                if(powerUp.getName() == "pill" && powerUp.intersects(playerShip.boundingBox)) {
                    ...
                    powerUpList.remove();
                    break;
                } else if(powerUp.getName() == "shield" && powerUp.intersects(playerShip.boundingBox)) {
                    playerShip.shield += 2;
                    powerUpList.remove();
                    break;
